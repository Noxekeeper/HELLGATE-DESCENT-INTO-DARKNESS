...
internal static class SinnerslaveCrossbowCombatAiPatch
{
    ...
    private static bool CanMeleeReach(float distY, float atkDistance)
    {
        float maxDy = Mathf.Max(0.25f, DoreiCombatAiConfig.MeleeMaxVerticalDelta);
        float reach = atkDistance > 0f ? atkDistance : 2f;
        return distY <= reach + maxDy;
    }

    private static void PreferRangedAttack(Traverse tr, float distY)
    {
        object atk = GetState("ATK");
        object atkDown = GetState("ATKDOWN");
        object atkUp = GetState("ATKUP");
        if (distY > DoreiCombatAiConfig.MeleeMaxVerticalDelta && atkDown != null)
        {
            tr.Field("state").SetValue(atkDown);
            tr.Field("Choose").SetValue(10);
        }
        else if (distY < -DoreiCombatAiConfig.MeleeMaxVerticalDelta && atkUp != null)
        {
            tr.Field("state").SetValue(atkUp);
            tr.Field("Choose").SetValue(10);
        }
        else if (atk != null)
        {
            tr.Field("state").SetValue(atk);
            tr.Field("Choose").SetValue(10);
        }
    ...
    public static bool AttackKind_Prefix(object __instance)
    {
        ...
        if (!CanMeleeReach(distY, atk))
        {
            PreferRangedAttack(tr, tr.Field("distance_y").GetValue<float>());
            return false;
        }

        if (dist < 2f && UnityEngine.Random.value <= DoreiCombatAiConfig.PreferMeleeOverFleeChance)
        {
    ...
    public static void moveout_Postfix(object __instance)
    {
        ...
        if (!CanMeleeReach(distY, atkDist))
        {
            PreferRangedAttack(tr, tr.Field("distance_y").GetValue<float>());
            return;
        }
    ...
    public static void fun_move_Postfix(object __instance)
    {
        ...
        if (state == null || (runState != null && state.Equals(runState)) || (blankState != null && state.Equals(blankState)))
        {
            float distY = Mathf.Abs(tr.Field("distance_y").GetValue<float>());
            float atk = tr.Field("Atkdistance").GetValue<float>();
            if (dist <= DoreiCombatAiConfig.MeleeRangeThreshold && CanMeleeReach(distY, atk) &&
    ...
    public static void fun_MOVE_Postfix(object __instance)
    {
        ...
        float atk = tr.Field("Atkdistance").GetValue<float>();
        if (dist > DoreiCombatAiConfig.MeleeRangeThreshold) return;
        if (!CanMeleeReach(distY, atk)) return;
    ...
    public static void Update_Postfix(object __instance)
    {
        ...
        // Target on another platform height: shoot, never rush into fake melee.
        if (!CanMeleeReach(distY, atk))
        {
            object stateEarly = tr.Field("state").GetValue();
            object deathEarly = GetState("DEATH");
            object dmgEarly = GetState("DAMAGE");
            object dmg2Early = GetState("DAMAGE2");
            object dmg3Early = GetState("DAMAGE3");
            object erowalkEarly = GetState("EROWALK");
            object stabblEarly = GetState("STABBLANK");
            if (stateEarly != null && (
                stateEarly.Equals(deathEarly) || stateEarly.Equals(dmgEarly) || stateEarly.Equals(dmg2Early) ||
                stateEarly.Equals(dmg3Early) || stateEarly.Equals(erowalkEarly) || stateEarly.Equals(stabblEarly)))
                return;
        ...
        // Very close and same height — force melee attack.
        if (dist < Mathf.Min(thr, atk + 1f))
        {
