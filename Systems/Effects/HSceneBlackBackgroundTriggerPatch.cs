...
internal static class HSceneBlackBackgroundTriggerPatch
{
    ...
    /// <summary>
    /// Tentacle H-scenes are excluded from black background processing:
    /// their vanilla visibility lifecycle is sensitive to external hide/show systems.
    /// </summary>
    private static bool IsTentacleEnemyType(string enemyType)
    {
        if (string.IsNullOrEmpty(enemyType)) return false;
        return enemyType.IndexOf("tentacle", StringComparison.OrdinalIgnoreCase) >= 0;
    }
    ...
    {
        // Special handling for InquisitionWhiteERO — PC transfer
        ...
        try
        {
            ...
            // Safety guard: do not run black background logic for any tentacle-type H-scene.
            if (IsTentacleEnemyType(enemyType))
            {
                return;
            }
            ...
            // BigoniBrother uses StartBigoniERO; detect by object name / parent / Bigoni component
            ...
            // CRITICAL: for ERO_iki and ERO_iki2 do not activate the black screen (may block events)
            if (isGoblin && isEroIkiAnim)
            {
                // Skip black-screen handling for ERO_iki/ERO_iki2
            ...
            // ERO_iki and ERO_iki2 should NOT trigger black background
            // CRITICAL: verify this is not an ERO_iki or ERO_iki2 animation
    ...
    private static void HandleInquisitionWhiteHandoff(InquisitionWhiteERO __instance, Spine.AnimationState state, Spine.Event e)
    {
        try
        {
            ...
            // Get spine and se_count via reflection
            ...
            // Check transfer conditions on JIGO (earlier than JIGO2 for seamlessness)
            if (eventName == "JIGO")
            {
                Plugin.Log.LogInfo($"[WHITE INQ] HANDOFF CONDITION MET: JIGO event (earlier for seamless transition), seCount={seCount}");

                // Check whether this enemy was already transferred
