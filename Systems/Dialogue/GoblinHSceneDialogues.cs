...
// Variable no longer used
    ...
    internal static void ProcessEvent(object goblinInstance, string animationName, string eventName, int seCount)
    {
        ...
        try
        {
            ...
            if (dialogues != null && dialogues.Length > 0)
            {
                ...
                // If this is a goblin dialogue, schedule a PC reply in 2 seconds
    ...
    /// <summary>
    /// Process ARADIA_RESPONSE and ARADIA_THOUGHT events
    /// </summary>
    ...
    /// <summary>
    /// Get dialogues for animation and event
    /// </summary>
    private static string[] GetDialogues(string animationName, string eventName, int seCount)
    {
        ...
        // First check segmented dialogues for SE events
        ...
        if (_dialogueData.ContainsKey(animKey))
        {
            ...
            // Normal dialogue lookup (now without Goblin* prefixes)
        }
        else
        {
            // Animation not found
    ...
    /// <summary>
    /// Start coroutine for PC reply in 2 seconds
    /// </summary>
    ...
    /// <summary>
    /// Coroutine for delayed PC reply
    /// </summary>
    private static System.Collections.IEnumerator DelayedAradiaResponse(object goblinInstance, string animationName, string coroutineKey)
    {
        ...
        // Determine reply type (RESPONSE for first phase, THOUGHT for second)
            ...
            if (animationData?.ContainsKey("events") == true)
            {
                ...
                if (events != null)
                {
                    foreach (var eventKvp in events)
                    {
                        ...
                        if (eventData != null)
                        {
                            // Process mindBroken levels (low, medium, high)
                            ...
                            {
                                // Decide where to store (responses or thoughts)
    ...
    /// <summary>
    /// Load ARADIA_RESPONSE animations
    /// </summary>
    ...
    /// <summary>
    /// Load ARADIA_THOUGHT animations
    /// </summary>
    ...
    /// <summary>
    /// Parse color from JSON dictionary
    /// </summary>
// ColorParser is used for color parsing

    /// <summary>
    /// Process onomatopoeia for goblins

    /// <summary>
    /// Get bone for goblin onomatopoeia
