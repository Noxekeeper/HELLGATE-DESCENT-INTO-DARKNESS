...
internal static class GrabThreatDialogues
{
    ...
    // REMOVED: Local playercon cache — use UnifiedPlayerCacheManager
    ...
    /// <summary>
    /// Manual JSON parsing (same approach as other project files)
    /// </summary>
    ...
    private static void ParseSettings(string json)
    {
        ...
        // Parse threatCooldownSeconds
            ...
            // Parse visibilityDistanceModifier
    ...
    /// <summary>
    /// Parse grabThreats section
    /// FIXED: Uses a more reliable regex for multiline arrays
    /// </summary>
    private static void ParseGrabThreats(string json)
    {
        string[] levels = { "mild", "medium", "extreme" };
        
        // Find grabThreats section
        ...
        // Find start of grabThreats object
        ...
        // Find end of grabThreats object (closing brace)
        int braceCount = 0;
        int braceEnd = -1;
        ...
        {
            // Find arrays for this level — use a simple string search
            ...
            // Find start of array (after colon)
    ...
    /// <summary>
    /// Parse perEnemyOverrides section (nested braces — old regex [^}]+ broke after the first enemy).
    /// </summary>
    ...
    /// <summary>
    /// RU/other languages often lack slavebigaxe — fill in phrases from EN (match WAVs in BigSlaveAxe).
    /// </summary>
    ...
    internal static void TryShowThreatOnIdle(EnemyDate enemy)
    {
        try
        {
            ...
            if (ShouldSuppressCombatThreats(enemy)) return;
    ...
    /// <summary>
    /// Legacy: Check and generate threat (kept for API compatibility; prefer TryShowThreatOnIdle)
    /// </summary>
    internal static void ProcessGrabThreat(object enemyInstance, string enemyType)
    {
        try
        {
            ...
            if (enemyInstance is EnemyDate edLegacy && !AttackSoundRegistry.IsThreatEnabledPrefab(edLegacy))
                return;
            if (enemyInstance is EnemyDate threatEnemy && ShouldSuppressCombatThreats(threatEnemy))
                return;
            ...
            // Check simultaneous threat limit
            CleanupExpiredThreats();
            ...
            // Determine threat level from MindBroken%
            string threatLevel = GetThreatLevel();
    ...
    /// <summary>
    /// Enemy visibility check (distance-based)
    /// Simplified: only ensure the enemy is not too far (up to 30 units)
    /// OPTIMIZED: Uses cached playercon and enemy.com_player
    /// </summary>
    private static bool IsEnemyVisible(object enemyInstance)
    {
        try
        {
            ...
            // PRIORITY 1: Use enemy.com_player directly (fastest path)
            ...
            {
                // Optimized: Use UnifiedPlayerCacheManager
        ...
        {
            return true; // On error, allow display
        }
    }
    
    /// <summary>
    /// Remove expired threats from the active list
    /// </summary>
    ...
    /// <summary>
    /// Determine threat level from MindBroken percent
    /// </summary>
    ...
    /// <summary>
    /// Pick a random threat from the matching category
    /// </summary>
    private static string SelectRandomThreat(string threatLevel, string enemyType)
    {
        try
        {
            ...
            // Try enemy-specific phrases first
    ...
    /// <summary>
    /// Display threat above enemy head
    /// </summary>
    private static void ShowThreat(object enemyInstance, string threat, string threatLevel)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(boneName)) return; // No bone — do not show; no fallbacks
            ...
            // Use static bone-bound display
            var display = DialogueFramework.GetDialogueDisplay();
            if (display != null)
            {
                // Call the static bone-bound version without movement animation
    ...
    /// <summary>
    /// Get bone name for a specific enemy type. Returns null if type unknown — no fallbacks.
    /// </summary>
    ...
    /// <summary>
    /// Get threat color by level
    /// </summary>
    private static Color GetThreatColor(string threatLevel)
    {
        return threatLevel switch
        {
            "mild" => new Color(1f, 0.8f, 0.8f, 1f),    // Light red
            "medium" => new Color(1f, 0.4f, 0.4f, 1f),  // Red 
            "extreme" => new Color(0.8f, 0f, 0f, 1f),   // Dark red
    ...
    /// <summary>
    /// Suppress grab/combat threat text and sounds when the enemy is not actually hostile
    /// toward the player (friendly reputation, bandits ignoring player, EventCore shell, etc.).
    /// </summary>
    internal static bool ShouldSuppressCombatThreats(EnemyDate enemy)
    {
        if (enemy == null || enemy.gameObject == null)
            return false;

        GameObject go = enemy.gameObject;
        bool hostileToPlayer = EnemyFactionRuntime.IsHostileToPlayer(go);

        EventCoreHost host = enemy.GetComponent<EventCoreHost>();
        if (host != null && host.ShouldSuppressCombatThreats(hostileToPlayer))
            return true;
        ...
        if (FactionReputationBehavior.ShouldSuppressVanillaAggro(go))
            return true;

        if (FactionReputationBehavior.ShouldBanditsIgnorePlayer(go))
            return true;

        return FactionReputationBehavior.GetLevelFor(go) != FactionReputationBehavior.Level.Hostile;
    }
