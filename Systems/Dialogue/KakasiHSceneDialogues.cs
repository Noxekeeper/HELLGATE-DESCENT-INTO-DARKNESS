...
// Colors are now taken from config
    ...
    private static void ParseAnimationComments(string jsonText)
    {
        try
        {
            ...
            // Cross animations (EroAnimation) — lowercase
            // Ground animations (kakashi_ero2) — UPPERCASE
            string[] animations = { 
                // Cross
    ...
    internal static void ProcessHSceneEvent(object enemyInstance, string animationName, string eventName, int seCount)
    {
        ...
        try
        {
            ...
            if (_lastCommentTime.ContainsKey(enemyInstance))
            {
                ...
                {
                    // Cooldown active — skip (logs disabled)
                    return;
                }
            }
            
            // Check maximum simultaneous comments
            CleanupExpiredComments();
            if (_activeComments.Count >= _maxSimultaneousComments)
            {
                // Maximum comments reached — skip (logs disabled)
                return;
            }
            
            // Normalize animation names (cross — lowercase, ground — UPPERCASE)
            string animKey = animationName;
            string eventKey = eventName;
            
            // For cross (EroAnimation) animation names are lowercase
            // For ground (kakashi_ero2) animation names are UPPERCASE
            if (typeName == "EroAnimation")
            {
                // Cross — normalize to lowercase
                animKey = animationName?.ToLowerInvariant() ?? "";
            }
            else
            {
                // Ground — normalize to UPPERCASE
                animKey = animationName?.ToUpperInvariant() ?? "";
            }
            
            // For SE events with se_count form key se_count_N
            if (eventName.Equals("SE", StringComparison.OrdinalIgnoreCase) && seCount > 0)
            {
                eventKey = $"se_count_{seCount}";
            }
            
            // Look up comments for this animation and event
            ...
            // First try lookup by eventKey (se_count_N or concrete event)
            List<string> phrases = null;
            if (events.ContainsKey(eventKey))
            {
                phrases = events[eventKey];
            }
            // If not found, try SE1, SE2, etc.
            ...
            // For cross: if SE event not found by se_count, try animation name as the event
            // Example: for animation "start" and SE event, look up event "start" in JSON
            else if (typeName == "EroAnimation" && eventName.Equals("SE", StringComparison.OrdinalIgnoreCase))
            {
                // Try to find an event named like the animation (start, start2, ero1, etc.)
                if (events.ContainsKey(animKey))
                {
                    phrases = events[animKey];
                }
            }
            // If the event is an animation name (e.g. START, ERO2 for ground)
            ...
            // Phrase selected and shown (logs disabled)
...
// ColorParser is used for color parsing
