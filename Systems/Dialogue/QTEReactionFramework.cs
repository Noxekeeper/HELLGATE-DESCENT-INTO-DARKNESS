...
// QTE reactions database loaded
    ...
    internal static void OnQTECorrect(int currentCombo, string enemyName = null)
    {
        ...
        // Pick category based on MindBroken level
    ...
    private static void OnQTEWrong()
    {
        ...
        // Get enemy name (if available)
        string enemyName = GetCurrentEnemyName();

        // Pick category based on MindBroken level
        string category = GetCategoryByMindBroken(false); // false = wrongPress (from PC)
    ...
    private static void OnQTEComboMilestone(int milestone)
    {
        ...
        // Ensure this is a new milestone (not a repeat)
        ...
        // Check probability (usually 100% for combo)
        float roll = UnityEngine.Random.Range(0f, 1f);
        ...
        {
            string phrase = phrases[UnityEngine.Random.Range(0, phrases.Length)];
            ShowEnemyReaction(phrase, null); // Combo reactions always come from the enemy
            _lastReactionTime = Time.time;
        }
    }

    /// <summary>
    /// Show enemy reaction (correctPress, comboMilestone) on enemy bones
    /// </summary>
    private static void ShowEnemyReaction(string text, string enemyName)
    {
        ...
        // Get enemyInstance for positioning
        object enemyInstance = GetCurrentEnemyInstance();
        if (enemyInstance == null)
        {
            // Fallback: use old method if enemyInstance is unavailable
            ShowReactionFallback(text);
            return;
        }

        // Use ShowTouzokuHSceneComment to display on enemy bones
        // Duration 3 seconds, font size 30px (enlarged for QTE reactions)
        // Offset 95px (70 + 25) to avoid overlapping other phrases
        // Red text with black outline for QTE mistakes
        display.ShowTouzokuHSceneComment(enemyInstance, text, 3f, 30f, 95f, 0f, new Color(1f, 0f, 0f, 1f), new Color(0f, 0f, 0f, 1f));
    }

    /// <summary>
    /// Show PC reaction (wrongPress) on PC bones
    /// </summary>
    private static void ShowPlayerReaction(string text, string enemyName)
    {
        ...
        // Get enemyInstance for positioning (needed to find PC bones)
        object enemyInstance = GetCurrentEnemyInstance();
        if (enemyInstance == null)
        {
            // Fallback: use old method if enemyInstance is unavailable
        ...
        // Use ShowAradiaHSceneComment to display on PC bones
        // Duration 3 seconds, font size 30px (enlarged for QTE), offset 75px (50 + 25), bone "face"
        // Offset increased by 25px to avoid overlapping other phrases
        // Red text with black outline for QTE mistakes
        display.ShowAradiaHSceneComment(enemyInstance, text, 3f, 30f, 75f, 0f, "face", new Color(1f, 0f, 0f, 1f), new Color(0f, 0f, 0f, 1f));
    }

    /// <summary>
    /// Fallback method to show reaction (if enemyInstance is unavailable)
    /// </summary>
    ...
    /// <summary>
    /// Pick reaction category based on MindBroken level
    /// </summary>
    private static string GetCategoryByMindBroken(bool isEnemyReaction)
    {
        ...
        if (isEnemyReaction)
        {
            // Enemy reactions (correctPress, comboMilestone)
            if (mindBrokenPercent < 0.5f) // 0–49% — low
            {
                // Bias toward dominant and taunting
                float roll = UnityEngine.Random.Range(0f, 1f);
                if (roll < 0.5f) return "dominant";
                if (roll < 0.8f) return "taunting";
                return "sexual";
            }
            else if (mindBrokenPercent < 0.8f) // 50–79% — medium
            {
                // Mixed categories
            ...
            {
                // Bias toward sexual and dominant
                float roll = UnityEngine.Random.Range(0f, 1f);
        ...
        {
            // PC reactions (wrongPress)
            if (mindBrokenPercent < 0.5f) // 0–49% — low
            {
                // Bias toward threatening and dominant (resistance)
                float roll = UnityEngine.Random.Range(0f, 1f);
            ...
            {
                // Bias toward sexual and submissive
                float roll = UnityEngine.Random.Range(0f, 1f);
    ...
    /// <summary>
    /// Get current enemy (if available)
    /// </summary>
    private static string GetCurrentEnemyName()
    {
        try
        {
            ...
            if (currentEnemyField != null)
            {
                ...
                if (enemyInstance != null)
                {
                    ...
                    // Common enemy types (from GrabProtectionPatch)
                    ...
                    // Inquisition types (may be BlackInquisitor)
                    else if (typeName.Contains("Inquisition") || typeName.Contains("InquisitionBlack"))
                    {
                        // Check specifically for BlackInquisitor
        ...
        return null; // Return null to use shared phrases
    }
