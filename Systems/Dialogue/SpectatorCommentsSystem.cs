...
internal static class SpectatorCommentsSystem
{
    ...
    /// <summary>
    /// Coroutine for H-scene monitoring and showing comments
    /// </summary>
    private static IEnumerator MonitorHSceneAndShowComments()
    {
        while (true)
        {
            ...
            // Check that an H-scene is active
            if (!IsHSceneActive())
            {
            ...
            // Determine H-scene stage
            string stage = GetHSceneStage();
            
            // Pick a random enemy for the comment
    ...
    private static List<EnemyDate> GetVisibleEnemiesInFrame()
    {
        ...
        try
        {
            ...
            {
                // Skip enemies currently in an H-scene
    ...
    /// <summary>
    /// Determine H-scene stage (start/ero/fin)
    /// </summary>
    private static string GetHSceneStage()
    {
        try
        {
            // Find the enemy currently in an H-scene
            EnemyDate[] allEnemies = UnityEngine.Object.FindObjectsOfType<EnemyDate>();
            ...
            foreach (EnemyDate enemy in allEnemies)
            {
                if (enemy.eroflag)
                {
                    // Get current animation via Spine
        ...
        // Fallback: return "ero" as the most likely stage
        return "ero";
    }
...
/// <summary>
/// Component for starting coroutines
/// </summary>
