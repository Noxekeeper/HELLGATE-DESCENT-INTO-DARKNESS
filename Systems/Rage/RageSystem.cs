...
internal static class RageSystem
{
    ...
    /// <summary>
    /// When enabled in config and Rage window is active: block grab paths and knockdown from power hit types (see <c>RageActiveImmunityPatch</c>).
    /// </summary>
    internal static bool IsGrabKnockdownImmuneWhileRageActive =>
        (Plugin.rageActiveImmuneGrabAndKnockdown?.Value ?? true) && Enabled && IsActive;

    // Config values
    private static float GainPerKill => Plugin.rageGainPerKill?.Value ?? 5f;
    private static float GainPerBossKill => Plugin.rageGainPerBossKill?.Value ?? 30.0f;
    ...
    {
        // Exactly 100% must count as Tier3-ready; strict >100 blocked full-bar activation from selecting Tier3 (wings, costs).
    ...
    /// <summary>
    /// Restores the passive Rage bar from save data. Active Rage window is always cleared.
    /// </summary>
    internal static void RestorePersistedBarPercent(float percent)
    {
        if (!Enabled) return;

        float max = Plugin.rageTier3OverflowThreshold?.Value ?? 103f;
        float old = _ragePercent;
        if (_isActive)
            Deactivate();

        _ragePercent = Mathf.Clamp(percent, 0f, max);
        _tier3ReadyUntilTime = 0f;
        if (!_isActive && _ragePercent > TIER3_UI_READY_START_PERCENT)
            _tier3ReadyUntilTime = Time.unscaledTime + TIER3_READY_GRACE_SECONDS;

        NotifyChanged(old, _ragePercent);
        try { RageComboSystem.ResetState(); } catch { }
    }
