...
internal static class RageVisualEffectsSystem
{
    ...
    // RAGE title popup (PNG)
    private static GameObject? rageTextObject;
    private static Image? rageTitleImage;
    private static Coroutine? textScaleCoroutine;
    private static Sprite? _rageTitleSprite;
    ...
    {
        return ResolveSourceFilePath(Path.Combine("Rage", "ha-ha-ha.wav"));
    }
    ...
    {
        return ResolveSourceDirectoryPath("RAGE_FURY");
    }
    ...
    private static Sprite? GetRageTitleSprite()
    {
        if (_rageTitleSprite != null)
            return _rageTitleSprite;

        try
        {
            string pngPath = ResolveSourceFilePath(Path.Combine("RAGE_FURY", "RAGE_UI.png"));
            ...
            _rageTitleSprite = Sprite.Create(
                tex,
        ...
        {
            Plugin.Log?.LogWarning($"[RAGE VISUAL] Failed loading RAGE_UI.png: {ex.Message}");
        }

        return _rageTitleSprite;
    }

    private static string ResolveSourceFilePath(string relativePathFromHellGateSources)
    {
        ...
        candidates[0] = Path.Combine(Path.Combine(gameRoot, "sources"), "HellGate_sources");
        candidates[1] = Path.Combine(Path.Combine(Path.Combine(gameRoot, ".."), "sources"), "HellGate_sources");
        candidates[2] = Path.Combine(Path.Combine(Path.Combine(Path.Combine(Path.Combine(gameRoot, "BepInEx"), "plugins"), "NoR_HellGate"), "sources"), "HellGate_sources");

        for (int i = 0; i < candidates.Length; i++)
        {
            string full = Path.Combine(candidates[i], relativePathFromHellGateSources);
            if (File.Exists(full))
                return full;
        }

        return Path.Combine(candidates[0], relativePathFromHellGateSources);
    }

    private static string ResolveSourceDirectoryPath(string relativePathFromHellGateSources)
    {
        string gameRoot = Application.dataPath;
        if (gameRoot.EndsWith("_Data"))
            gameRoot = gameRoot.Substring(0, gameRoot.Length - 5);
    ...
    {
        if (canvasRect == null) return;
        Sprite? titleSprite = GetRageTitleSprite();
        if (titleSprite == null)
        {
            // Do not spawn placeholder rectangles when the PNG is missing/unreadable.
            return;
        }
        ...
        if (rageTextObject == null)
        {
            ...
            rageTitleImage = rageTextObject.AddComponent<Image>();
            
            // Ensure object is active before setup.
            rageTextObject.SetActive(true);
            
            rageTitleImage.sprite = titleSprite;
            rageTitleImage.type = Image.Type.Simple;
            rageTitleImage.preserveAspect = true;
            rageTitleImage.color = Color.white;
            rageTitleImage.raycastTarget = false;
            textRect.sizeDelta = new Vector2(300f, 200f);
        }
        else
        {
            rageTextObject.SetActive(true);
            if (rageTitleImage != null)
            {
                rageTitleImage.sprite = titleSprite;
                rageTitleImage.color = Color.white;
            }
    ...
    {
        if (rageTextObject == null || rageTitleImage == null) yield break;
        ...
        // Keep title visible for the entire active Rage window (tier durations are managed by RageSystem)
        // with a subtle breathing scale effect.
        float pulseTime = 0f;
        rageTitleImage.color = Color.white;
        while (RageSystem.IsActive && rageTextObject != null && rageTitleImage != null)
        {
            pulseTime += Time.deltaTime;
            float pulse = 1f + 0.06f * Mathf.Sin(pulseTime * 6.5f);
            rageTextObject.transform.localScale = new Vector3(pulse, pulse, 1f);
            rageTitleImage.color = Color.white;
            yield return null;
        }
