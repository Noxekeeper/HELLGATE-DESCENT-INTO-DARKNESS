...
/// <summary>
/// Blue glow on bone3, bone8 during Time Slow-Mo (T).
/// </summary>
internal static class SlowMoBoneGlowSystem
{
    private const string RunnerObjectName = "SlowMoBoneGlowRunner_XUAIGNORE";
    private const string CanvasObjectName = "SlowMoBoneGlowCanvas_XUAIGNORE";

    private static Color GlowColor => new Color(
        Plugin.slowMoBoneGlowColorR?.Value ?? 0.3f,
        Plugin.slowMoBoneGlowColorG?.Value ?? 0.6f,
        Plugin.slowMoBoneGlowColorB?.Value ?? 1f,
        Plugin.slowMoBoneGlowAlpha?.Value ?? 0.85f
    );
    private static float GlowSizePx => Plugin.slowMoBoneGlowSizePx?.Value ?? 48f;
    ...
    private static SlowMoBoneGlowRunner? _runner;
    ...
    internal static void Initialize()
    {
        ...
        {
            Plugin.Log?.LogError($"[SlowMoBoneGlow] Init error: {ex.Message}");
        }
    }

    private static void OnSlowMoActivated()
    {
        if (!(Plugin.slowMoBoneGlowEnable?.Value ?? true)) return;
        try
        {
            EnsureCanvas();
            EnsureRunner();
        ...
        {
            Plugin.Log?.LogError($"[SlowMoBoneGlow] Activate error: {ex.Message}");
        }
    }
    ...
    private static void OnSlowMoDeactivated()
    {
        ...
        {
            Plugin.Log?.LogError($"[SlowMoBoneGlow] Deactivate error: {ex.Message}");
        }
    }
    ...
    private static void EnsureRunner()
    {
        ...
        _runner = runnerObj.AddComponent<SlowMoBoneGlowRunner>();
    ...
    private static void CreateGlowObjects()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            var go = new GameObject($"SlowMoBoneGlow_{BoneNames[i]}_XUAIGNORE");
...
internal class SlowMoBoneGlowRunner : MonoBehaviour
{
    ...
    {
        if (_canvasRect != null && TimeSlowMoSystem.IsActive)
            SlowMoBoneGlowSystem.UpdatePositions(_canvasRect);
    }
}
