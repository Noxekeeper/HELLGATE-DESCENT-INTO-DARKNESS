...
public static class QTESystem {
    ...
    private static float _tentacleProbeNextTime = -1f;
    private static bool _tentacleProbeCachedActive = false;
    ...
    private static System.Reflection.FieldInfo GetInPraymaidenField() {
        if (!triedResolvePraymaidenField) {
            ...
            var type = HellGateTypeResolver.Resolve("NoREroMod.PlayerConPatch");
            if (type != null) {
    ...
    public static void StartQTE(PlayerStatus playerStatus, playercon playerCon) {
        ...
        TryResolveCurrentEnemyInstance();
    ...
    public static void StopQTE() {
        ...
        // Hide Canvas and buttons (keep pooled UI to avoid hitch on next struggle window)
        HideButtons();
        if (qteCanvas != null) {
    ...
    public static void Update(PlayerStatus playerStatus, playercon playerCon) {
        ...
        // Aradia is dead: struggle/QTE must never run. Otherwise the player can keep
        // filling SP and "get up" from a fatal grab (vanilla cleared _SOUSA, but QTE
        // re-enables it through TryForceStruggleEscape). Hard-stop here.
        if (playerCon._Death || playerStatus.Hp <= 0f) {
            if (isQTEActive) {
                StopQTE();
            }
            return;
        ...
        if (isQTEActive) {
            ...
            if (playerStatus.Sp >= playerStatus.AllMaxSP() &&
                ...
                !IsTentacleHSceneActiveFast(playerCon))
            {
                if (TryForceStruggleEscape(playerCon, playerStatus))
                {
                    StopQTE();
                    return;
                }
    ...
    /// <summary>
    /// Fast enemy lookup for H-scene QTE (one FindObjectsOfType&lt;EnemyDate&gt; instead of per-ERO-type scans).
    /// </summary>
    private static void TryResolveCurrentEnemyInstance()
    {
        if (currentEnemyInstance != null)
            return;
        ...
        try
        {
            ...
            EnemyDate fallbackActiveEro = null;
            for (int i = 0; i < enemies.Length; i++)
            {
                ...
                if (ed.eroflag)
                {
                    currentEnemyInstance = ed;
                    return;
                }

                if (ed.erodata != null && ed.erodata.activeInHierarchy)
                    fallbackActiveEro = ed;
            }

            if (fallbackActiveEro != null)
                currentEnemyInstance = fallbackActiveEro;
        }
        catch (Exception ex)
        {
            LogWarning("[QTE] Failed to resolve enemy for QTE: " + ex.Message);
        }
    }

    private static void EnsureButtons()
    {
        if (leftButton != null && rightButton != null && upButton != null && downButton != null && statusBar != null)
            return;
    ...
    private static Vector2 GetButtonRowCenterAnchorPosition()
    {
        float yFromTop = Mathf.Clamp(ButtonPositionYFromTop, 0f, ReferenceScreenHeight);
        float yFromBottom = ReferenceScreenHeight - yFromTop;
        return new Vector2(ButtonPositionX, yFromBottom);
    }
    ...
    /// </summary>
    internal static bool TryForceStruggleEscape(playercon pc, PlayerStatus ps) {
        if (pc == null || ps == null)
            return false;
        if (pc.erodown == 0)
            return true;

        if (!BirthRecoveryStruggleState.CanForceStruggleEscape(pc, ps))
            return false;
        ...
        try {
            ...
            Traverse.Create(pc).Field("key_item").SetValue(false);
        } catch (Exception ex) {
        ...
        if (pc.erodown == 0 && pc.eroflag)
        {
            ...
            NoREroMod.Patches.Enemy.HellishTouzokuModCustom.HellishTouzokuHSceneEscapePatch
                .AbortActiveHellishTouzokuHSceneOnPlayerEscape(pc, requireErodownClear: true);
            NoREroMod.Patches.Enemy.MummyDogPassPatch.AbortActiveMummyDogHSceneOnPlayerEscape(
                pc,
                requireErodownClear: true);
            NoREroMod.Patches.Trap.TrapdataHSceneEscapePatch.AbortActiveTrapHScenesOnPlayerEscape(
                pc,
                requireErodownClear: true);
            NoREroMod.Patches.Enemy.BossTouzokuCustom.BossTouzokuCustomHSceneEscapePatch
                .AbortActiveFieldHSceneOnPlayerEscape(pc, requireErodownClear: true);
            NoREroMod.Patches.Player.PlayerCombatControlRecovery.RestoreAfterStruggleEscape();
        }

        return pc.erodown == 0;
    }

    private static bool IsBlackOozeTrapHSceneActive() {
        try {
            foreach (BlackOozetrap trap in UnityEngine.Object.FindObjectsOfType<BlackOozetrap>()) {
                if (trap != null && trap.eroflag)
                    return true;
            }

            foreach (BlackOozeTrapTypeB trap in UnityEngine.Object.FindObjectsOfType<BlackOozeTrapTypeB>()) {
                if (trap != null && trap.eroflag)
                    return true;
            }
        } catch {
    ...
    private static bool IsTentacleHSceneActiveFast(playercon pc)
    {
        if (pc == null || !pc.eroflag || pc.erodown == 0)
        {
            _tentacleProbeCachedActive = false;
            _tentacleProbeNextTime = -1f;
            return false;
        }

        // Fast path: if we already know current enemy, avoid scene-wide scans.
        if (currentEnemyInstance != null)
        {
            string enemyName = currentEnemyInstance.GetType().Name;
            if (enemyName.IndexOf("Tentacle", StringComparison.OrdinalIgnoreCase) >= 0 ||
                enemyName.IndexOf("Ironmaiden", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                _tentacleProbeCachedActive = true;
                _tentacleProbeNextTime = Time.unscaledTime + 0.75f;
                return true;
            }
        }

        float now = Time.unscaledTime;
        if (_tentacleProbeNextTime > now)
        {
            return _tentacleProbeCachedActive;
        }

        _tentacleProbeNextTime = now + 0.75f;
        _tentacleProbeCachedActive = false;

        try
        {
            // Core tentacle enemy (EnemyDate) with eroflag field.
            Type? tentacleType = HellGateTypeResolver.Resolve("Tentacle");
            if (tentacleType != null)
            {
                FieldInfo? eroflagField = AccessTools.Field(tentacleType, "eroflag");
                UnityEngine.Object[] tentacles = UnityEngine.Object.FindObjectsOfType(tentacleType);
                foreach (UnityEngine.Object t in tentacles)
                {
                    if (t == null) continue;
                    if (eroflagField?.GetValue(t) is bool ero && ero)
                    {
                        _tentacleProbeCachedActive = true;
                        return true;
                    }
            ...
            // ERO controllers used by tentacle traps (check active state).
            string[] tentacleEroTypes =
            {
                "Tentacles_ero",
            ...
            foreach (string typeName in tentacleEroTypes)
            {
                Type? eroType = HellGateTypeResolver.Resolve(typeName);
                if (eroType == null) continue;
                UnityEngine.Object[] eros = UnityEngine.Object.FindObjectsOfType(eroType);
                foreach (UnityEngine.Object e in eros)
                {
        ...
        {
            // Keep QTE stable even if reflection probing fails on some builds.
        }
    ...
    private static void TryActivateRageDuringQTE() {
        try {
            ...
            if (!TryForceStruggleEscape(currentPlayerCon, currentPlayerStatus)) {
