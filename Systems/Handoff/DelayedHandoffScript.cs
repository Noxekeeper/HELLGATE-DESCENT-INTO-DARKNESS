...
public class DelayedHandoffScript : MonoBehaviour {
    ...
    public void StartDelayedHandoff(object enemyInstance) {
        float delay = Plugin.handoffDelay.Value; // Use setting from config
    ...
    private IEnumerator DelayedHandoffCoroutine(object enemyInstance, float delay) {
        // Plugin.Log.LogInfo($"[DELAYED HANDOFF] Delay {delay} seconds before handoff...");
        
        // Wait the specified time (REAL TIME - not affected by slow-mo!)
        yield return new WaitForSecondsRealtime(delay);
        
        // Plugin.Log.LogInfo($"[DELAYED HANDOFF] Delay finished, handing off the protagonist!");
        
        // Resolve enemy type and call the matching handoff method
        string enemyType = enemyInstance?.GetType()?.Name ?? "Unknown";
        // Plugin.Log.LogInfo($"[DELAYED HANDOFF] Enemy type: {enemyType}");
        
        // Call the handoff function depending on enemy type
        if (enemyType == "InquiBlackEro") {
            ...
            // For swordsmen (EroTouzoku) and remaining types, use TouzokuNormalPassPatch
