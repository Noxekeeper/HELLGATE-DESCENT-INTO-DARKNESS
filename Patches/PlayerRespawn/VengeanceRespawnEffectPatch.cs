...
internal static class VengeanceRespawnEffectPatch
{
    ...
    internal static void ApplyVengeanceEffects()
    {
        ...
        try
        {
            NoREroMod.Systems.Pregnancy.ShelterAttack.ShelterAttackDriver.ResolveDefeatIfAssaultActive();
            PregnancyVengeanceCleanup.ClearOnTakeVengeance();

            LethalTrapDeathCleanup.ForceCleanupForRespawn();
            HSceneEscapeStateCleanup.RestoreAfterBadEndVengeance();

            // Any in-place vengeance revive (lethal trap OR mob/enemy death) skips the
            // PlayerStatus.REstrat / Restart altar-redirect prefixes, so we must handle
            // cross-zone altar homes here. Arm only when active zone ≠ VanillaAltarCatalog
            // home (savepoint/coords) — e.g. death in InundergroundChurch with a scapegoat
            // save. Local IUC altar saves (InUnder / InUnder_over) do NOT latch.
            NoREroMod.Patches.Player.VanillaCutsceneSceneGuard.NotifyPotentialNoAltarZoneDeath();
            NoREroMod.Patches.Player.VanillaEvSceneExitPatch.RequestNoAltarZoneAltarExit();
            ...
            LethalTrapVengeanceMindBrokenShock.TryStartAfterVengeance();
