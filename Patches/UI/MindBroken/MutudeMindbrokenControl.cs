...
internal static class MutudeMindbrokenControl
{
    ...
    private static bool BoostBadstatus(Collider2D col, MultipleAtkBadstatus __instance)
    {
        try
        {
            ...
            // Fetch private fields via Harmony
            ...
            if (count > 1f)
            {
                ...
                if (pl != null)
                {
                    // Base value 35 -> multiply by 3 (200% boost)
            ...
            // Skip the original method because we handled it ourselves
            return false;
        }
        catch (Exception ex)
        {
            Plugin.Log?.LogWarning($"[MutudeMindbrokenControl] Error boosting badstatus: {ex.Message}");
            // On error, do not block the original method
            return true;
        }
    }
}
