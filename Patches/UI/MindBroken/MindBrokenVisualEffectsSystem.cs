...
internal static class MindBrokenVisualEffectsSystem
{
    ...
    internal const string OverlayCanvasObjectName = "MindBrokenVisualEffectsCanvas_XUAIGNORE";
    ...
    {
        if (!IsEnabled) return;
        if (MindBrokenSystem.IsScriptedSequenceActive) return;
        try {
    ...
    {
        // Milestones are no longer used for the negative effect; logic moved to OnMindBrokenPercentChanged
        // Keep the method for compatibility, but do not use it
    }
    ...
    private static void EnsureOverlayCanvas()
    {
        ...
        try
        {
            ...
            if (existing != null)
            {
                ...
                // Use existing canvas (logging disabled)
                return;
            }
            ...
            canvas.sortingOrder = 999; // Below CorruptionCaptions (1000)
            ...
            // Canvas created (logging disabled)
        }
        catch (Exception ex)
        {
        }
    ...
    private static void EnsureCameraEffects()
    {
        try
        {
            // Retry finding the camera; it may not be ready during initialization
            if (mainCamera == null)
            {
            ...
            // Simple pink flash via UI Image
    ...
    private static IEnumerator RetryFindCamera()
    {
        ...
        while (mainCamera == null && attempts < 20)
        {
            ...
            if (cameraObj != null)
            {
                ...
                if (mainCamera != null)
                {
                    // Camera found on attempt {attempts} (logging disabled)
                    EnsureCameraEffects();
                    yield break;
                }
            }
            ...
            {
                mainCamera = Camera.main;
                // Camera.main found on attempt {attempts} (logging disabled)
                EnsureCameraEffects();
                yield break;
            }
        }
    ...
    // Simple negative effect for 1 second
    ...
    // Legacy milestone method (unused; kept for compatibility)
    private static void TriggerNegativeEffect(float percent)
    {
        // Ensure camera and component are ready
        if (mainCamera == null)
        {
            EnsureCameraEffects();
        }
        ...
        try
        {
            // Choose parameters based on percent
            // Use ranges because milestones can be 0.5, 0.75, 0.9, 1.0
            float duration = 0f;
            float frequency = 0f;
            int blinkCount = 0;
            
            if (percent >= 0.9f) // 90% and above (including 100%)
            {
                duration = 0.6f;
                frequency = 4f;
                blinkCount = 6;
            }
            else if (percent >= 0.75f) // 75% and above (e.g. 80%)
            {
                duration = 0.5f;
                frequency = 3f;
                blinkCount = 5;
            }
            else if (percent >= 0.5f) // 50% and above
            {
                duration = 0.4f;
                frequency = 2.5f;
                blinkCount = 4;
            }
            else if (percent >= 0.3f) // 30% and above
            {
                duration = 0.3f;
                frequency = 2f;
                blinkCount = 3;
            ...
            {
                return; // Do not start for other percents
            }
            ...
            // Negative effect started (logging disabled)
        }
        catch (Exception ex)
        {
        }
    ...
    private static IEnumerator NegativeBlinkCoroutine(float duration, float frequency, int blinkCount)
    {
        ...
        for (int i = 0; i < blinkCount; i++)
        {
            ...
            if (i < blinkCount - 1) // Do not wait after the last blink
            {
