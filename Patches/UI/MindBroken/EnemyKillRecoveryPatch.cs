...
/// <summary>
/// Patch for tracking enemy kills via OnDestroy on specific classes.
/// Safe approach: patch only concrete enemy classes, not MonoBehaviour.
/// </summary>
[HarmonyPatch]
internal static class EnemyKillRecoveryPatch
{
    // Patch concrete enemy classes
    ...
    private static void TouzokuNormal_OnDestroy_Postfix(TouzokuNormal __instance)
    {
        try
        {
            ...
            {
                MindBrokenRecoverySystem.RegisterKill(__instance);
            }
        }
        catch { }
    }
