...
namespace NoREroMod.Patches.Effects
{
    ...
    internal static class PregnancyClipTrigger
    {
        ...
        private static void OnCreampieStart(PlayerStatus __instance)
        {
            try
            {
                // When the extended Pregnancy module owns the creampie scale, the vanilla
                // CreampieVal_UI no longer means "conception" (it fires on every creampie).
                // The module shows the conception clip itself at womb-full, so stand down here.
        ...
        /// <summary>
        /// Shows the conception clip exactly once. Called by the Pregnancy module at the
        /// moment the womb fills (actual conception), during the active H-scene.
        /// </summary>
        public static void ShowConceptionClip()
        {
            try
            {
                _shown = false;
            ...
            {
                Plugin.Log?.LogWarning($"[PregnancyClipTrigger] conception clip error: {ex}");
            }
        }
