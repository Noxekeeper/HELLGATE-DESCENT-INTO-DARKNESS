...
/// Hands off control after the timed section completes (END animation).
/// </summary>
...
/// <summary>
/// Crow Inquisitor receives the player after a handoff and starts from START.
/// No closed loop — always ends on END/END2.
/// </summary>
    ...
    private const float SpeechCooldown = 5f; // Cooldown for Crow speech lines
    ...
    protected override bool IsCycleComplete(string animationName, string eventName, int seCount)
    {
        ...
        // CrowInquisition has no closed loop — completes on END animations
    ...
    protected override void ForceAnimationToMiddle(SkeletonAnimation spine)
    {
        ...
        try
        {
            ...
            // Crow Inquisitor: rewind to START (scene beginning)
            var track = spine.AnimationState.SetAnimation(0, "START", false);
            if (track?.Animation != null)
            {
                track.Time = 0f; // Start from the beginning
            }
        }
        catch (Exception ex)
        {
