...
class GoblinPassLogic : BaseEnemyPassPatch<goblinero>
{
    ...
    protected override bool IsCycleComplete(string animationName, string eventName, int seCount)
    {
        ...
        // Handoff only after the second phase (2ERO_JIGO)
    ...
    private static void GoblinPass(goblinero __instance, Spine.Event e, int ___se_count)
    {
        ...
        try
        {
            ...
            // Goblin uses Base.TrackCycles, which checks EnemyHandoffSystem.GlobalHandoffCount
            ...
            // Process onomatopoeia based on sounds from animation events
            try {
    ...
    /// <summary>
    /// Process onomatopoeia for goblin animation events
    /// </summary>
    private static void ProcessOnomatopoeiaForEvent(goblinero goblinInstance, string animationName, string eventName, int seCount)
    {
        ...
        if (!NoREroMod.Systems.Dialogue.SoundRegistry.HasSound(soundKey))
            return;
    ...
    private static string GetSoundKeyForEvent(string animationName, string eventName, int seCount)
    {
        ...
        switch (animationName)
        {
            ...
            case "2ERO_JIGO":
                // End event - add a suitable sound
                if (eventName == "SE" && seCount == 1) return "ero_enemy_syasei1";
                break;
        }

        return null; // No matching sound
    }
    ...
    /// <summary>
    /// Push the player away from the enemy (analogous to TouzokuNormalPassPatch)
    /// Goblins do NOT inherit from EnemyDate, so we use a direct approach
    /// </summary>
    private static void PushPlayerAwayFromEnemy(object enemyInstance)
    {
        ...
        try
        {
            ...
            // Stop enemy H-animation (analogous to Touzoku)
            try
            {
                // Get enemy spine via reflection (in goblinero the field is myspine)
            ...
            // Hide enemy (analogous to Touzoku)
            try
            {
            ...
            // Get playercon and set eroflag = false (exit H-scene)
    ...
    /// <summary>
    /// Static fields for handoff UI phrases
    /// </summary>
    ...
    /// <summary>
    /// Public accessors for private fields from external classes
    /// </summary>
    ...
    /// <summary>
    /// Display player-handoff message for goblins (above enemy head)
    ...
    protected static void DisplayHandoffMessage(string message, float displayTime, object enemyInstance)
    {
        try
        {
            ...
            if (handoffUI == null)
            {
                ...
                canvas.sortingOrder = 999; // Same as grab phrases
                ...
                handoffText.font = NoREroMod.Systems.UI.HellGateFontProvider.GetUiFont();
                handoffText.fontSize = 44;
                handoffText.alignment = TextAnchor.MiddleCenter;
                handoffText.color = new Color(1f, 0.2f, 0.2f); // Red
                handoffText.fontStyle = FontStyle.Bold;
                
                // Component for position tracking
                handoffUI.AddComponent<GoblinHandoffPositionTracker>();
            }
            
            // Update text and activate UI
            if (handoffText != null)
            {
    ...
    /// <summary>
    /// For goblins on handoff use 2ERO_START instead of ERO2
    /// </summary>
    protected override void ForceAnimationToMiddle(Spine.Unity.SkeletonAnimation spine)
    {
        try
        {
            ...
            // For goblins on handoff start at 2ERO_START
            const string middleAnim = "2ERO_START";
            const bool isLoop = false; // START animations are usually not looped
...
/// <summary>
/// MonoBehaviour that hides handoff phrases after a delay
/// </summary>
...
/// <summary>
/// MonoBehaviour that tracks enemy position and updates handoff phrase position
/// </summary>
public class GoblinHandoffPositionTracker : MonoBehaviour
{
    void Update()
    {
        ...
        if (handoffRect != null && handoffUI != null)
        {
            ...
            screenPos.y += 120f; // Upward offset above enemy head
    ...
    /// <summary>
    /// Reset all goblin data (called on escape from the gangbang cycle)
    /// By analogy with TouzokuNormalPassPatch
    /// </summary>
    internal static void ResetAll()
    {
        try
        {
            // Call ResetAll on the base class via reflection
            ...
            // Clear handoff-phrase UI elements via reflection
