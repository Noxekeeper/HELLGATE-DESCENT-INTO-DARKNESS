...
/// <summary>
/// Patch for InquisitionBlack - механику handoff of GG after 2 циклов
...
class InquisitionBlackPassPatch {
    ...
    static void InquisitionBlackPass(InquiBlackEro __instance, Spine.Event e, int ___se_count) {
        try {
            ...
            // (показ X-ray баннера for инквизитора удален in пользу нового общits подхода)
            
            // Processing dialogue system events
            try {
                string eventName = e?.Data?.Name ?? e?.ToString() ?? string.Empty;
                
                // Processing кастомных phrases InquisitionBlack during H-scene
                // IMPORTANT: Для START, START2 force call обработку with именем animation
                if (currentAnim == "START" || currentAnim == "START2")
                {
                    ...
                    // For InquisitionBlack always вызываем system Арадии after processing phrases enemy
                ...
                // Check if this is event event переключения animation
                ...
                // IMPORTANT: Для FIN/FIN2 используем имя events as имя animation, since currentAnim еще not обновился
                if (isAnimationSwitchEvent && (currentAnim != "START" && currentAnim != "START2"))
                {
                    string targetAnim = currentAnim;
                    
                    // If event переключения animation (FIN, FIN2 etc.), используем имя events as имя animation
                    ...
                    {
                        targetAnim = eventName; // Use имя events as имя animation
                    }
                    
                    // Process event переключения animation with именем animation as event
                    NoREroMod.Systems.Dialogue.InquisitionBlackHSceneDialogues.ProcessHSceneEvent(
                        __instance,
                        targetAnim, // Use correctlyе имя animation
                        targetAnim, // Use имя animation as имя events
                        0
                    );
                ...
                // IMPORTANT: События переключения animation (FIN, FIN2, ERO etc.) already обработаны выше
    ...
    /// Список соответствует реальным анимациям from InquiBlackEro.cs
    /// </summary>
    private static bool IsHAnimation(string animationName) {
        if (string.IsNullOrEmpty(animationName)) return false;
        
        // Список H-анимаций for InquisitionBlack (from исходного кода InquiBlackEro.cs)
        string[] hAnimations = {
            "START", "START2",
    ...
    private static bool IsCycleComplete(string animationName, string eventName, int seCount) {
        // Завершение цикла инквизитора — строго on JIGOFIN (not JIGOFIN2)
    ...
    /// Возможные варианты: JIGO, ERO, ERO1, ERO2
    /// </summary>
    private static void ForceAnimationToMiddle(SkeletonAnimation spine) {
        try {
            if (spine == null) return;
            
            // For InquisitionBlack: start with START2 (skip only START, короткая прелюдия)
            // Plugin.Log.LogInfo($"[ANIMATION] Force InquisitionBlack to START2 (short intro)");
            
            // Clear текущую animation и устанавливаем START2
        ...
        try {
            ...
            if (enemyComponent != null) {
                try {
                    ...
                    // Make enemy неvisibleым (as in original - скрываем only erodata)
...
/// <summary>
/// MonoBehaviour for корутины скрытия сообщений
/// </summary>
