...
namespace NoREroMod.Patches.Enemy.Six_hand
{
    internal static class MutudeEffects
    {
        ...
        private static bool playOnce = false; // Flag for one-shot playback
        ...
        internal static void PlayVideo(string videoDirectory, Vector2 anchoredPosition, Vector2 size, float rotationDegrees = 0f, bool loop = true)
        {
            try
            {
                ...
                // One-shot playback (loop = false): shrink size by 40% (multiply by 0.6)
                if (!loop)
                {
                ...
                playOnce = !loop; // Set one-shot playback flag
        ...
        private static List<string> BuildCandidateDirectories(string videoDirectory)
        {
            ...
            {
                // FIN1 uses CUM_inside_normal; FIN2 uses demon_cum_000
        ...
        private static IEnumerator FramePlayback()
        {
            ...
            while (videoActive && frameTextures.Count > 0)
            {
                ...
                // One-shot playback with all frames shown — stop
                ...
                // Loop only when not in one-shot playback
                if (!playOnce)
                {
    ...
    {
        // Simple MonoBehaviour subclass used to start coroutines
    }
}
