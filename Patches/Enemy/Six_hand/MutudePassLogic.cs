...
{
    /// <summary>
    /// Handoff logic and cycle tracking for the Six Hands demon (Mutude).
    ...
    internal class MutudePassLogic : BaseEnemyPassPatch<Mutudeero>
    {
        ...
        private static readonly Dictionary<object, float> soundEffectCooldown = new(); // Cooldown for sound effects
        private const float SpeechCooldown = 4f; // Cooldown for Mutude speech
        ...
        protected override void ForceAnimationToMiddle(SkeletonAnimation spine)
        {
            ...
            try
            {
                ...
                // Forced-animation logs disabled
            }
            catch (Exception ex)
            {
            }
        ...
        private static void MutudePass(Mutudeero __instance, Spine.AnimationState state, int trackIndex, Spine.Event e)
        {
            ...
            try
            {
                ...
                // Ensure this is Mutude's spine, not the player's
                if (spine.transform.parent != null && spine.transform.parent.name.Contains("Player"))
                {
                    // Try to locate Mutude's correct spine
                ...
                // eventName already declared above
        ...
        private static void ClearStateOnImmediatelyERO()
        {
            try
            {
                // Always clear state, even if the enemy is already disabled
                // Ensures the next Mutude spawn starts in a correct state
                ClearStateData();
            }
            catch (Exception ex)
            {
        ...
        private static void PushPlayerAwayFromEnemy(object enemyInstance)
        {
            try
            {
                ...
                if (playerCon != null)
                {
                    ...
                    // Push the player away from the enemy (same as Touzoku)
                    ...
                    if (enemyTransform != null)
                    {
                        ...
                        // Force-clear vertical velocity to avoid launching the player
                ...
                if (mutudeEro != null)
                {
                    ...
                    if (owner != null)
                    {
                        try
                        {
                            ...
                            // DO NOT disable owner.gameObject — the main Mutude must stay active,
                            // so it can grab the player again after release (same as Touzoku)
                            // owner.gameObject.SetActive(false); // REMOVED
                        }
                        catch (Exception ex)
                        {
                        }
        ...
        // New modular dialogue system will be built from scratch
