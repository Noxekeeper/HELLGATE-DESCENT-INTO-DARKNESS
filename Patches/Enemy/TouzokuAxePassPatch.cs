...
class TouzokuAxePassPatch {
    ...
    static void TouzokuAxePass(EroTouzokuAXE __instance, Spine.Event e, int ___se_count) {
        try {
            ...
            try {
                ...
                // Process custom TouzokuAxe phrases during H-scene
                // IMPORTANT: For START–START5 force-process using the animation name
    ...
    /// List matches real animations from EroTouzokuAXE.cs
    /// </summary>
    private static bool IsHAnimation(string animationName) {
        if (string.IsNullOrEmpty(animationName)) return false;
        
        // H-animation list for TouzokuAxe (from EroTouzokuAXE.cs source)
        string[] hAnimations = {
    ...
    private static bool IsCycleComplete(string animationName, string eventName, int seCount) {
        // TouzokuAxe: FIN is climax with se_count==1, then FIN2, FIN3, JIGO, JIGO2
        // Ideal handoff point is after FIN (climax)
    ...
    /// For TouzokuAxe: use end of cycle (FIN2/FIN3/JIGO) for correct idle
    /// </summary>
    private static void ForceAnimationToMiddle(SkeletonAnimation spine) {
        try {
            if (spine == null) return;
            
            // For TouzokuAxe: start from end of cycle so idle loads correctly
            // DO NOT use ERO2; only final animations after climax
            const string endAnim = "JIGO";
            const bool isLoop = false; // JIGO animation is not looped
            
            // Plugin.Log.LogInfo($"[ANIMATION] Force TouzokuAxe to end cycle: {endAnim} (loop={isLoop})");
            
            // Clear current animation and set an end-of-cycle animation
                ...
                try {
                    ...
                    if (enemySpineField != null) {
                        ...
                        if (enemySpine != null) {
                            ...
                            // Try different idle animation variants for TouzokuAxe
                    ...
                    // Need to restore the main enemy model (instead of erodata)
                    // erodata is a child object; the main model is the parent
    ...
    static void ClearStateOnImmediatelyERO() {
        try {
            // Check enemy type - clear only for TOUZOKU AXE
            EroTouzokuAXE currentEnemy = Object.FindObjectOfType<EroTouzokuAXE>();
            if (currentEnemy == null)
            {
                // Not a TOUZOKU AXE enemy - do not clear
                return;
            }
