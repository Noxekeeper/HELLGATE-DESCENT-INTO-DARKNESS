...
/// <summary>
/// Patch for InquisitionRedERO - player handoff after 2 cycles
/// </summary>
class InquisitionRedPassPatch : BaseEnemyPassPatch<InquisitionRedERO>
{
    protected override string EnemyName => "InquisitionRed";

    protected override int CyclesBeforePass => 1; // Handoff after 1 full cycle (until JIGO)
    ...
    private static void InquisitionRedPass(InquisitionRedERO __instance, Spine.Event e, int ___se_count)
    {
        ...
        try
        {
            ...
            // REMOVED: "first capture" block reset animation to START whenever currentAnim != "START".
            // Original OnEvent runs BEFORE our Postfix — by the time the patch runs the game has already
            // advanced to START2/START3 etc. Forced reset to START broke the chain and created
            // an infinite loop: START → START2 → reset to START → repeat.
    ...
    protected override void ForceAnimationToMiddle(SkeletonAnimation spine)
    {
        try
        {
            ...
            // Use START2 (like InquisitionBlack) — without complex ERO4_2 count logic
