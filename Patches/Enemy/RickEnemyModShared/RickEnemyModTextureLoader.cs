...
internal sealed class RickEnemyModTextureLoader : TextureLoader
{
    private readonly string _imagesDir;
    private readonly Material _materialTemplate;
    private readonly string _name;

    public RickEnemyModTextureLoader(string imagesDir, Material materialTemplate, string name)
    {
        _imagesDir = imagesDir ?? "";
        _materialTemplate = materialTemplate;
        _name = name ?? "RickEnemyMod";
    }
    ...
    public void Load(AtlasPage page, string path)
    {
        ...
        {
            Plugin.Log?.LogError($"[RickEnemyMod] PNG not found: {fullPath}");
            return;
        }
        ...
        {
            Plugin.Log?.LogError($"[RickEnemyMod] Failed to load image: {fullPath}");
            return;
        }
