...
internal sealed class HellishTouzokuTextureLoader : TextureLoader
{
    private readonly string _imagesDir;
    private readonly Material _materialTemplate;
    private readonly string _name;

    public HellishTouzokuTextureLoader(string imagesDir, Material materialTemplate, string name)
    {
        _imagesDir = imagesDir ?? string.Empty;
        _materialTemplate = materialTemplate;
        _name = name ?? "HellishTouzoku";
    }
    ...
    public void Load(AtlasPage page, string path)
    {
        ...
        {
            Plugin.Log?.LogError($"[HellishTouzokuTextureLoader] PNG not found: {fullPath}");
            return;
        }
        ...
        {
            Plugin.Log?.LogError($"[HellishTouzokuTextureLoader] Failed to load image: {fullPath}");
            return;
        }
