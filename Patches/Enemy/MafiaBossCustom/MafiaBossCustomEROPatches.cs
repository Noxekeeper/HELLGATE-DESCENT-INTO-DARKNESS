...
/// <summary>
/// EroMafiamuscle patches: for MafiaBossCustom (by oya name) block BadEnd on 4ERO and hand off on 1EROFIN2 / 3EROJIGO / 4EROJIGO.
/// On the first ERO event, if currentAnim==START and globalHandoffCount>0 — swap to STARTERO/2ERO/4ERO (3EROJIGO removed — glitchy).
...
internal static class MafiaBossCustomEROPatches
{
    ...
    // Critical optimization: cache all FieldInfo (hit on every Spine event!)
    ...
    /// <summary>
    /// Block BadEnd: for MafiaBossCustom, fadeevent() does not run.
    /// </summary>
    ...
    /// <summary>
    /// Postfix on OnEvent: on 1EROFIN2 / 3EROJIGO / 4EROJIGO start handoff by variant.
    /// 
    /// IMPORTANT: START → STARTERO/2ERO/4ERO swap now happens in OnTriggerStay2D,
    /// to avoid double work (START starts → interrupted → new animation)
    /// </summary>
    ...
    private static void OnEvent_Postfix(EroMafiamuscle __instance, Spine.Event e)
    {
        try
        {
            ...
            // Remove BadEnd "Press any button" canvas and flag — not needed for MafiaBossCustom (handoff instead of bad end)
