...
/// <summary>
/// Spine <see cref="TextureLoader"/> that maps atlas pages to Materials built from on-disk PNGs.
/// </summary>
internal sealed class SlaveBigAxeMeatArmorTextureLoader : TextureLoader
{
    private readonly string _imagesDir;
    private readonly Material _materialTemplate;
    private readonly string _name;

    public SlaveBigAxeMeatArmorTextureLoader(string imagesDir, Material materialTemplate, string name)
    {
        _imagesDir = imagesDir ?? "";
        _materialTemplate = materialTemplate;
        _name = name ?? "SlaveBigAxeMeatArmor";
    }
    ...
    public void Load(AtlasPage page, string path)
    {
        ...
        {
            Plugin.Log?.LogError($"[SlaveBigAxeMeatArmor] PNG not found: {fullPath}");
            return;
        }
        ...
        {
            Plugin.Log?.LogError($"[SlaveBigAxeMeatArmor] Failed to load image: {fullPath}");
            return;
        }
    ...
    {
        // Atlas lifetime is owned by the cached SkeletonDataAsset; nothing to dispose per page.
    }
}
