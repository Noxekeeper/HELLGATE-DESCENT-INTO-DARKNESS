...
/// <summary>
/// Blocks original Bigoni Game Over mechanics for BigoniBrother.
/// Keeps player control after the H-scene and prevents loading the defeat scene.
/// </summary>
[HarmonyPatch]
internal static class BigoniBrotherGameOverBypass
{
    internal static void Apply(Harmony harmony)
    {
        ...
        foreach (Type nested in typeof(BigoniBrotherGameOverBypass).GetNestedTypes(BindingFlags.NonPublic))
    ...
    {
        return BigoniBrotherIdentity.ShouldBypassVanillaGameOver(instance, out bigoni);
    }
    ...
    /// <summary>
    /// Ensures the player is active, physics is enabled, and struggle is available.
    /// </summary>
    ...
    /// <summary>
    /// Overload without StartBigoniERO — used after handoff.
    /// </summary>
    ...
    private static void EnsurePlayerReady(playercon player, PlayerStatus status)
    {
        if (player != null)
        {
            ...
            // If the heroine is down, keep nowdamage true so state remains DOWN
    ...
    private static string GetEventName(Spine.Event e)
    {
        ...
        // Spine 3.8 uses the name property (lowercase)
        var data = e.Data;
        return data.Name;
    }

    /// <summary>
    /// Track BigoniBrother H-scene start and keep the player enabled.
    /// </summary>
    ...
    /// <summary>
    /// Reset flag after handoff of the player to another enemy.
    /// </summary>
    ...
    /// <summary>
    /// Block BigoniERO.Start, which prepares the Game Over scene.
    /// </summary>
